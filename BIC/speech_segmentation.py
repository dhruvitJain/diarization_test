...
def multi_segmentation(file, sr, frame_size, frame_shift, plot_seg=False, save_seg=False, cluster_method=None):
    ...
    mfccs = librosa.feature.mfcc(y, sr, n_mfcc=20, hop_length=frame_shift, n_fft=frame_size)
    ...
    if save_seg:
        ...
        for i in range(len(save_segpoint)-1):
            ...
            librosa.output.write_wav("SampleData/%s.wav" % i, tempAudio, sr)
    ...
    if cluster_method == 'kmeans':
        ...
        vq_features = np.zeros((len(classify_segpoint) - 1, 20), dtype=np.float32)
        for i in range(len(classify_segpoint) - 1):
            tempAudio = y[classify_segpoint[i]:classify_segpoint[i + 1]]
            mfccs = librosa.feature.mfcc(tempAudio, sr, n_mfcc=20, hop_length=frame_shift, n_fft=frame_size)
